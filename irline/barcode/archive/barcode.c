...
char* get_barcode_char() {
    ...
    */
    ...
    // Ensure that the length of the barcode(9), and number of wide bars(3) is correct
    if((strlen(scanned_code) == CODE_LENGTH) && (white_bar[1] + black_bar[1] == 3)) {
    ...
    // if((strlen(scanned_code) == CODE_LENGTH) && (black_bar[0] + black_bar[1] == 5) && (white_bar[0] + white_bar[1] == 4) && (white_bar[1] + black_bar[1] == 3)) { // Complex checks
...
bool read_barcode(struct repeating_timer *t) {
    ...
    if((current_color != last_scanned_color) && (voltage_difference >= MIN_VOLTAGE_DIFF) && (time_diff >= min_time_narrow_bar)) {
        ...
        // START SCAN or SCANNING
            ...
            scanned_timings[strlen(scanned_code)] = time_diff;
